...
const EnvSchema = z
  .object({
    ...
    // Admin portal & IAM (sub-plan A1, Task 1). Staff identity lives on a Google Workspace domain,
    // and the portal refuses any address outside it — see the plan's Decisions table.
    ADMIN_WORKSPACE_DOMAIN: z.string().min(1).default('amana-ng.com'),
    // The first owner, seeded from config and never minted by an endpoint (invariant 6). Because it
    // is config, it is the one address that can exist before anyone has been onboarded.
    ADMIN_BOOTSTRAP_OWNER_EMAIL: z.string().email().default('david@amana-ng.com'),
    // A staff session is absolute, not sliding: 8 hours, then sign in again. Shorter than the
    // 30-day customer refresh token on purpose — this session can suspend a business, and the
    // second gate (Cloudflare Access) re-checks Google on its own schedule anyway.
    ...
    // Must match the redirect URI registered on the OAuth app, exactly.
    ADMIN_OIDC_REDIRECT_URI: z.string().url().default('http://localhost:3000/admin/auth/callback'),
    // Where the browser lands after signing in or being refused.
    ADMIN_PORTAL_URL: z.string().url().default('http://localhost:3400'),
  })
  // The portal refuses any address outside `ADMIN_WORKSPACE_DOMAIN` (sub-plan A1, invariant: the
  // Workspace is the identity boundary). An owner configured outside it is therefore an owner who
  // can never sign in — a system that looks configured and admits nobody, discovered only when
  // someone tries. Cheaper to refuse the boot, in every environment, since it is equally wrong in
  // all of them.
  .superRefine((cfg, ctx) => {
    const domain = cfg.ADMIN_WORKSPACE_DOMAIN.toLowerCase();
    if (cfg.ADMIN_BOOTSTRAP_OWNER_EMAIL.toLowerCase().split('@')[1] !== domain) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['ADMIN_BOOTSTRAP_OWNER_EMAIL'],
        message: `ADMIN_BOOTSTRAP_OWNER_EMAIL must be on the Workspace domain (${domain})`,
      });
    }
  });
