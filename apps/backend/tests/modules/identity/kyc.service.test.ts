...
describe('kyc.service.shouldRecommendKycUpgrade', () => {
  it('returns false for an agent (no balance, no tier promotion path)', () => {
    expect(shouldRecommendKycUpgrade({ role: 'agent', kycTier: '1', balanceKobo: kobo(0n) })).toBe(
      false,
    );
  });
  ...
  it('returns true for principal at Tier 2 above 80% of cap', () => {
    ...
    expect(
      shouldRecommendKycUpgrade({ role: 'principal', kycTier: '2', balanceKobo: kobo(eightyPct) }),
    ).toBe(true);
  });
  ...
  it('returns false for principal at Tier 2 below 80% of cap', () => {
    ...
    expect(
      shouldRecommendKycUpgrade({ role: 'principal', kycTier: '2', balanceKobo: kobo(halfCap) }),
    ).toBe(false);
  });
