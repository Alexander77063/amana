...
Monorepo: pnpm workspaces (`pnpm@10.33.2`, Node `>=20`) + Turborepo. Five apps — `apps/{backend,principal,agent,retailer-portal,admin-portal}` — and four shared packages (`packages/{types,api-client,ui,validation}`). Biome for format/lint (single quotes, 2-space, 100-col). Backend tests are Vitest against a **real Postgres** (PostGIS) — no DB mocking.

The two portals are **Next.js 14 App Router web apps, not React Native**, and neither consumes `@amana/ui` (which ships raw RN source that Metro transpiles): each duplicates the design tokens once in its own `app/globals.css`, tracked in the design brief's §9.2 drift table. `apps/retailer-portal` (:3300) is for merchants; `apps/admin-portal` (:3400) is for Amana staff. **The admin portal holds no secrets** — its only config is `BACKEND_ORIGIN` (server-side, never `NEXT_PUBLIC_*`) — and it answers `/admin/*`, `/vendors-admin/*` and `/retailers/*` itself through explicit Route Handler proxies over `lib/proxy.ts`, forwarding them to the API, because the staff session cookie is **host-only**. Its own pages must therefore stay off those three paths (`/`, `/sign-in`, `/ops/*`, `/people/*`), and every screen renders from the permissions `/admin/me` returns, never from a role name. See `docs/runbook/admin-portal.md`.
...
Canonical schema: **`apps/backend/src/env.ts`** (Zod, parsed at import, throws a formatted list on failure). Most vars have dev-safe defaults. Production essentials with **no safe default**, all **enforced at boot** (`loadEnv` throws in `NODE_ENV=production` if missing): `JWT_SECRET` (≥32 chars), `FIELD_ENCRYPTION_KEY` (64 hex), `ANCHOR_API_KEY`, `ANCHOR_WEBHOOK_SECRET`, `TERMII_API_KEY`, `GOOGLE_OAUTH_CLIENT_ID` + `GOOGLE_OAUTH_CLIENT_SECRET` (the Workspace OAuth app — since A1 Task 4 this is the ONLY way into the ops surfaces, so a missing one means no claim queue, no retailer KYB, no suspensions) (dev fallbacks/optionality apply only outside production); `DEV_OTP_BYPASS_CODE` is rejected in production. Before go-live, register the Termii sender ID and flip `ANCHOR_API_BASE_URL` off the sandbox default — see `docs/runbook/go-live-checklist.md`. AWS region for S3 media is `af-south-1` (`MEDIA_BUCKET=amana-media-af-south-1`).
...
`docs/product/` (**the ten foundation docs and their status — start here**), `docs/adr/` (decisions), `docs/superpowers/plans/` (sub-plan implementation docs), `docs/superpowers/specs/` (design spec), `docs/runbook/` (`local-dev.md`, `anchor-sandbox.md`, `funds-model.md` — the limits-only sub-wallet funds model & money flows, `vas.md` — digital VAS (airtime/data/electricity/cable) money model, cash-out control & webhook flow, `retailer-onboarding.md` — marketplace retailer state machine, admin auth, `kyb.*` webhooks & the retailer portal, `marketplace-buyer.md` — buyer browse, the category-vs-section distinction & the merchant-rule control fusion, `vendor-registry.md` — the passive vendor registry's sweep phases, env vars, shadow-data query & per-household enforcement switch, `vendor-claim.md` — the vendor claim rail: the 202 non-oracle rule, the two ownership proofs, working the ops claim queue, the enforcement-switch API, suspending a vendor & its pre-launch OTP/race gates, `google-workspace-setup.md` — the staff Workspace domain, the OAuth app & the break-glass stand-down, `admin-portal.md` — the staff portal: the same-origin proxy & why it is a Route Handler, what each permission renders, the two A1 Task 5 permission changes, the `amana-admin` deploy sequence & the known limits, `go-live-checklist.md` — pre-production readiness), `docs/brainstorm/locked-decisions.md`.
