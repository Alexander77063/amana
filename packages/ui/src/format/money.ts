 ...
 * 1 naira = 100 kobo. Pure integer (BigInt) math — the value is never coerced to a float —
 * so it is exact at any magnitude. Always renders 2 decimal places.
 * Assumes a non-negative amount (kobo strings from our backend always are).
 *
 * Shared by both mobile apps (`@amana/principal`, `@amana/agent`) — the single source of truth
 * for kobo→naira display formatting.
 */
