...
export const anomalyService = {
  score(
    ...
    const weighted =
      (features[0] as { name: string; value: number }).value * weights.amount_zscore +
      (features[1] as { name: string; value: number }).value * weights.hour_of_day +
      (features[2] as { name: string; value: number }).value * weights.vendor_novelty +
      (features[3] as { name: string; value: number }).value * weights.velocity;
