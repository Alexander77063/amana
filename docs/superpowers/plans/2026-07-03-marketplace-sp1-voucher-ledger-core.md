...
- **Redemption settle** (webhook `transfer.completed`, kind=redemption → `redemptionSettlementService.finalise`): ONE balanced entry `debit suspense = discounted, credit external = retailerNet, credit commission = commission` (discounted = retailerNet + commission). `payoutStatus=paid`.

**Commission ledger account (sign convention — resolved).** The spend flow recognises its fee by **debiting** the credit-normal `fee` LA (`settlement.service` debits it; `refund.service` credits it to reverse). Marketplace commission must NOT share that account — it is carved *from* the payment (not charged on top) and mixing it would corrupt spend-fee reporting and the negative-master recon alert. Use a **dedicated `commission` ledger-account kind** (credit-normal; revenue recognised by crediting it — textbook), added to `ledger_account_kind` in migration 0027 and **lazily provisioned per master wallet on first use** exactly as `settlement.service` already lazy-creates the `external` LA (no backfill), plus added to `masterWalletsRepo.provision` for new wallets.
