...
describe('rule engine evaluate', () => {
  it('allow when no rules', () => {
    expect(
      evaluate(intent(), { id: 'rs', subWalletId: 'sw', version: 1, rules: [] }, ctx()),
    ).toEqual({
      kind: 'allow',
    });
  });
  ...
  it('allow when all rules pass', () => {
    const rules: Rule[] = [
      { id: 'r1', kind: 'limit', priority: 10, config: { windowKind: 'daily', maxKobo: 100_000n } },
      {
        ...
        kind: 'category',
        priority: 20,
        config: { mode: 'allowlist', categories: ['groceries'] },
      },
    ];
  ...
  it('require_bump when one rule fails; reason matches that rule', () => {
    ...
    const out = evaluate(
      intent({ amountKobo: kobo(40_000n) }),
  ...
  it('collects all failures across multiple rules; firstFailedReason follows priority', () => {
    ...
    const out = evaluate(
      intent({ amountKobo: kobo(40_000n), category: 'groceries' }),
      {
        id: 'rs',
        subWalletId: 'sw',
        version: 1,
