...
export const adminIamRoute = new Hono<{ Variables: AdminActorVariables }>()
  ...
  .post('/admins/:id/roles', async (c) => {
    ...
    const proposal = await adminIamService.grantRole(db, {
      actorAdminUserId: actor.adminUserId,
      targetAdminUserId: params.id,
      role: body.role,
      reason: body.reason ?? null,
    });
    // 202, not 204: since Task 3 a grant is a REQUEST. It needs a second admin before it means
    // anything, and the response says so rather than letting the caller assume it landed. The
